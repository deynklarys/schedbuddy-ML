...
# Define and parse user input argument
...
parser.add_argument('--train_pct', help='Ratio of images to go to train folder; ' \
                    'the rest go to validation folder (example: ".8)', default=0.8)
...
if not os.path.isdir(data_path):
    print("Directory specified by --datapath not found. Verifythe path is correct (and use double back slashes of on Windows) and try again.")
    sys.exit(0)
...
cwd = os.getcwd()
train_img_path = os.path.join(cwd, 'data\\train\\images')
train_txt_path = os.path.join(cwd, 'data\\train\\labels')
...
for i, set_num in enumerate([train_num, val_num]):
    for ii in range(set_num):
        ...
        if os.path.exists(txt_path): # If txt path does not exist, this is a background image, so skip the txt file
